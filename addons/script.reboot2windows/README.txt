...
This addon will reboot your OpenELEC machine once into windows then boot back into OpenELEC. It uses GRUB so a working multi-boot setup with GRUB is required. This addon uses GRUB 2.00 and has been tested with GRUB 1.99 and 2.00. Updates to GRUB can break this addon.

Manual editing files contained within the zip is required to get this addon working (preferably before installation).

script.reboot2windows\bin\reboot2windows.sh
...
You should now be back in OpenELEC. Now select 'windows' under Programs to use the addon and reboot once to the OS of your choice.

Reboot2windows is based on Reboot2oos-oe
